				...
				if (a != null)
				{
					...
					if (EntityDebug != null)
					{
						...
						if (DebugComponent == null)
							Debug.LogError("Failed to get the DebugComponent!");
					}
				...
				if (EntityDebug != null)
				{
					if (DebugComponent != null)
					{
						if (DebugComponent.active)
						{
							...
							if(desiredEntity.Equals(Entity.Null))
							{
								Debug.LogWarning("Desired Entity is empty!");
								entityIsValid = false;
							}else if(EntityManager.GetName(desiredEntity) != DebugComponent.EntityName)
							{
								Debug.LogWarning("Detected an entity change please wait while I fetch a new one");
								entityIsValid = false;
							}
							if (!entityIsValid)
							{
								...
								else if (CoreDataQuery.CalculateEntityCount() > 0)
								{
									...
									{
										Debug.Log("LiveEntityDebugSystem: Successfully found the entity\n\""+EntityManager.GetName(desiredEntity)+"\":\""+DebugComponent.EntityName+"\"");
									}
								}
