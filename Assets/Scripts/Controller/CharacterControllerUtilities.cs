...
namespace Pokemon.EntityController
{
	// Stores the impulse to be applied by the character controller body
	public struct DeferredCharacterControllerImpulse : IComponentData
	{
	...
	public static class CharacterControllerUtilities
	{
		...
		public const float k_SimplexSolverEpsilonSq = k_SimplexSolverEpsilon * k_SimplexSolverEpsilon;
		...
		public static float3 Gravity = new float3(0,-9.81f,0);
		...
		public const float MaxMovementSpeed = 10.0f;
		...
		public const float JumpUpwardsSpeed = 5.0f;

		// Maximum slope angle character can overcome (in degrees)
		public const float MaxSlope = 60.0f;
		...
		public const float CharacterMass = 1.0f;
		...
		public const float ContactTolerance = 0.1f;

		// Whether to affect other rigid bodies
		public const int AffectsPhysicsBodies = 1;
		...
		public static unsafe void CheckSupport(
			ref PhysicsWorld world, PhysicsWorld stepInputWorld, ref PhysicsCollider collider,ref EntityControllerStepInput stepInput, RigidTransform transform, float maxSlope,
			ref StateData stateData, out float3 surfaceNormal, out float3 surfaceVelocity, float maxSpeed,float3 up)
		{
			...
			//preform collision calculation
			{
				bool isValid = true;
				if (stepInput.ContactTolerance == 0) { UnityEngine.Debug.LogWarning("Contact tolerance is 0!"); isValid = false; }
				if (stepInput.RigidBodyIndex == -1) { UnityEngine.Debug.LogWarning("Given Entity Index is invalid!"); isValid = false; }
				if (float.IsNaN(transform.pos.x)) { UnityEngine.Debug.LogWarning("transform is null or invalid!"); isValid = false; }
				if (collider.ColliderPtr == null) { UnityEngine.Debug.LogWarning("Collider is null or invalid!"); isValid = false; }
				if (isValid)
				{
			...
			{
				UnityEngine.Debug.LogWarning("distance hit collector returns 0");
				stateData.supported = false;
				return;
			}
	//		else UnityEngine.Debug.LogWarning("distance hit collector returns more than 0");
			
			// Downwards direction must be normalized
			// We'll use the Camera Up vector for now
			float3 downwardsDirection = -up;
			...
			for (int i = 0; i < distanceHitsCollector.NumHits; i++)
			{
				...
				CreateConstraint(stepInputWorld, up,
			...
			{
				float velAlongDownwardsDir = math.dot(stepInput.Velocity.Linear, downwardsDirection);
				bool velocityIsAlongDownwardsDirection = velAlongDownwardsDir > 0.0f;
				if (velocityIsAlongDownwardsDirection)
				{
					UnityEngine.Debug.Log("velocity is downward");
				...
				{
					UnityEngine.Debug.Log("velocity is not downwards");
			...
			SimplexSolver.Solve(stepInput.DeltaTime, stepInput.DeltaTime, up, maxSpeed, constraints, ref outPosition, ref outVelocity, out float integratedTime, false);


		// obselete
		//	SimplexSolver.Solve(stepInputWorld, stepInput.DeltaTime, stepInput.DeltaTime, stepInput.Up, maxSpeed,
		//		constraints, ref outPosition, ref outVelocity, out float integratedTime, false);
			...
			// Check support state
			{
				UnityEngine.Debug.Log(initialVelocity+","+ outVelocity+"."+(initialVelocity - outVelocity)+",(" + k_SimplexSolverEpsilonSq+"),("+math.lengthsq(outVelocity)+")\n"+ math.lengthsq(initialVelocity - outVelocity)+","+ 
					(math.lengthsq(initialVelocity - outVelocity)<k_SimplexSolverEpsilonSq)+","+ (math.lengthsq(outVelocity) < k_SimplexSolverEpsilonSq)+"\n"+numSupportingPlanes);
				if (math.lengthsq(initialVelocity - outVelocity) < k_SimplexSolverEpsilonSq)
				{
					stateData.supported = true;
		...
		/// <summary>
		/// Collides and intergrates collision
		/// </summary>
		/// <param name="tStepInput"></param>
		...
		public static unsafe void CheckSupport(
		ref PhysicsWorld world, ref PhysicsWorld stepInputWorld,ref PhysicsCollider collider,ref EntityControllerStepInput stepInput, RigidTransform transform,
		out float3 surfaceNormal, out float3 surfaceVelocity, float maxSpeed,float3 up,
		NativeList<StatefulCollisionEvent> collisionEvents = default)
		{
			...
			Assert.IsTrue(Unity.Physics.Math.IsNormalized(up));
			...
			var maxDisplacement = -stepInput.ContactTolerance * up;
			{
				ColliderCastInput input = new ColliderCastInput()
				{
					Collider = collider.ColliderPtr,
			...
			float3 outVelocity = initialVelocity;
			float3 outPosition = transform.pos;
			SimplexSolver.Solve(stepInput.DeltaTime, stepInput.DeltaTime, up, maxSpeed,
				constraints, ref outPosition, ref outVelocity, out float integratedTime, false);
			...
			int numSupportingPlanes = 0;
			{
				for (int j = 0; j < constraints.Length; j++)
				{
					...
					if (constraint.Touched && !constraint.IsTooSteep && !constraint.IsMaxSlope)
					{
						...
						{
							var collisionEvent = new StatefulCollisionEvent(stepInputWorld.Bodies[stepInput.RigidBodyIndex].Entity,
								stepInputWorld.Bodies[constraint.RigidBodyIndex].Entity, stepInput.RigidBodyIndex, constraint.RigidBodyIndex,
			...
			// Check support state
			{
				if (math.lengthsq(initialVelocity - outVelocity) < k_SimplexSolverEpsilonSq)
				{
					...
					stepInput.stateData.supported = false;
					stepInput.stateData.isSliding = false;
				}
				else if (math.lengthsq(outVelocity) < k_SimplexSolverEpsilonSq && numSupportingPlanes > 0)
				{
				...
				else
				{
					// Check if sliding
					outVelocity = math.normalize(outVelocity);
					float slopeAngleSin = math.max(0.0f, math.dot(outVelocity, -up));
					float slopeAngleCosSq = 1 - slopeAngleSin * slopeAngleSin;
					if (slopeAngleCosSq <= maxSlopeCos * maxSlopeCos)
					{
						stepInput.stateData.supported = true;
						stepInput.stateData.isSliding = true;
					}
					else if (numSupportingPlanes > 0)
					{
						stepInput.stateData.supported = true;
