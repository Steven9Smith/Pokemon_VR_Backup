...
namespace Core.Spawning {
	...
	/*	public static void SetPokemonMoveRenderMesh(EntityManager entityManager,Entity entity, string pokemonMoveName)
		...
		//				if (pmde.isValid) Debug.Log("It be valid!");
				...
				entityManager.AddComponentData<CoreData>(entity,new CoreData(new ByteString30(name),new ByteString30("PokemonMove"),cd.size,cd.scale,entity, cd.translation, cd.rotation, pmds.physicsDamping));
		...
		//		Debug.Log("AAAAAAAAAAAAA "+a);
				...
				switch (name)
				{
					case "ThunderBolt":
						...
						CoreFunctionsClass.SetRenderMesh(entityManager, entity, pokemonName,1);
						break;
					case "Tackle":
						entityManager.SetName(entity, name);
						break;
