...
namespace Core
{
	...
	public struct CoreData : IComponentData
	{
		...
		public CoreData(ByteString30 name, ByteString30 baseName, float3 mSize, float3 mScale, Entity _entity, Translation _translation, Rotation _rotation, PhysicsDamping pd)
		{
			Name = name;
			BaseName = baseName;
			size = mSize;
		...
		public CoreData(ByteString30 name, ByteString30 baseName, Transform transform, Mesh mesh, Entity _entity, Translation _translation, Rotation _rotation, PhysicsDamping pd)
		{
			Name = name;
			BaseName = baseName;
			entity = _entity;
		...
		public CoreData(ByteString30 name, ByteString30 baseName, GameObject go, Entity _entity, Translation _translation, Rotation _rotation, PhysicsDamping pd)
		{
			Name = name;
			BaseName = baseName;
			entity = _entity;
		...
		public void SetPhysicsDamping(PhysicsDamping pd)
		{
			damping = pd;
		}
		public void SetTranslations(Translation position)
		{
	...
	public class CoreFunctionsClass
	{
		...
		/// <summary>
		/// preforms a NaN check on a float3
		/// </summary>
		/// <param name="a">float3</param>
		/// <returns></returns>
		public static bool IsNaN(float3 a) { return float.IsNaN(a.x) || float.IsNaN(a.y) || float.IsNaN(a.z); }
		/// <summary>
		/// preforms a NaN check on a float4
		/// </summary>
		/// <param name="a">float3</param>
		/// <returns></returns>
		public static bool IsNaN(float4 a) { return float.IsNaN(a.x) || float.IsNaN(a.y) || float.IsNaN(a.z) || float.IsNaN(a.w); }
		/// <summary>
		/// preforms a NaN check on a quanterion
		/// </summary>
		/// <param name="a">float3</param>
		/// <returns></returns>
		public static bool IsNaN(quaternion a) { return float.IsNaN(a.value.x) || float.IsNaN(a.value.y) || float.IsNaN(a.value.z) || float.IsNaN(a.value.w); }
		...
		// Functions that set components in Entities
		/// <summary>
		...
		{
			RenderMesh tmp = new RenderMesh { };
			return SetRenderMesh(entityManager, entity, name,out tmp, type);
		}
		/// <summary>
		/// gets the render mesh for any gameobject (that has been coded) and set the RenderMesh Component Data
		/// </summary>
		...
		/// <param name="entity">entity to be modified</param>
		/// <param name="name">base name of the entity</param>
		/// <param name="renderMesh">renderMesh to be moddified</param>
		/// <param name="type">type of entity 0 = Pokemon, 1 = PokemonMove</param>
		/// <returns></return>
		public static bool SetRenderMesh(EntityManager entityManager, Entity entity, ByteString30 name,out RenderMesh renderMesh, int type = 0)
		{
			GameObject go = null;
			string path = "";
			switch (type)
			{
				...
				default:
					Debug.LogWarning("Failed to load the render mesh for \"" + name + "\"");
					renderMesh = new RenderMesh { };
					return false;
			}
			// get the Prefab as a GameObject
		...
		/// <summary>
		/// Sets the Physics Damping for each given entity based on name and type
		/// </summary>
		/// <param name="entityManager">EntityManager</param>
		...
		/// <returns></returns>
		public static PhysicsDamping SetPhysicsDamping(EntityManager entityManager, Entity entity, ByteString30 name, PokemonEntityData ped)
		{
		...
		/// <summary>
		/// Locates a GmaeObject based on its type and name
		/// </summary>
		/// <param name="name">Name of the Prefab</param>
		/// <param name="type">type determines location
		/// 0 = Pokemon
		/// 1 = PokemonMoves
		/// 2 = Enviroment
		/// </param>
	...
	/// <summary>
	/// This makes a request to have the name of the entity be the CoreData's "Name:BaseName"
	/// </summary>
	public struct CoreDataNameRequest : IComponentData
	{
	...
	/// <summary>
	/// add this to an entity for it to be destroyed naturally
	/// </summary>
