...
namespace Pokemon.EntityController
{
	...
	public static class CharacterControllerUtilities
	{
		...
		public static unsafe void CheckSupport(
			ref PhysicsWorld world, PhysicsWorld stepInputWorld, ref PhysicsCollider collider, EntityControllerStepInput stepInput, RigidTransform transform, float maxSlope,
			ref StateData stateData, out float3 surfaceNormal, out float3 surfaceVelocity, float maxSpeed)
		{
			surfaceNormal = float3.zero;
			surfaceVelocity = float3.zero;
			...
			{
				DistanceHit hit = distanceHitsCollector.AllHits[i];
				CreateConstraint(stepInputWorld, stepInput.Up,
					hit.RigidBodyIndex, hit.ColliderKey, hit.Position, hit.SurfaceNormal, hit.Distance,
					stepInput.SkinWidth, maxSlopeCos, ref constraints);
			}
			...
			float3 initialVelocity;
			{
				float velAlongDownwardsDir = math.dot(stepInput.LinearVelocity, downwardsDirection);
				bool velocityIsAlongDownwardsDirection = velAlongDownwardsDir > 0.0f;
				if (velocityIsAlongDownwardsDirection)
				{
					float3 downwardsVelocity = velAlongDownwardsDir * downwardsDirection;
		...
		public static unsafe void CollideAndIntegrate(PhysicsWorld World, PokemonEntityData ped,
					EntityControllerStepInput stepInput, float characterMass, bool affectBodies, Collider* collider,
					ref RigidTransform transform, ref float3 linearVelocity, ref NativeStream.Writer deferredImpulseWriter)
		{
			// Copy parameters
			float deltaTime = stepInput.DeltaTime;
			...
			for (int i = 0; i < stepInput.MaxIterations && remainingTime > timeEpsilon; i++)
			{
				...
				// Do a collider cast
				{
					...
					{
						ColliderCastHit hit = collector.AllHits[hitIndex];
						CreateConstraint(World, stepInput.Up,
							hit.RigidBodyIndex, hit.ColliderKey, hit.Position, hit.SurfaceNormal, hit.Fraction * math.length(displacement),
							stepInput.SkinWidth, maxSlopeCos, ref constraints);
					}
				}
				...
				float3 prevVelocity = newVelocity;
				float3 prevPosition = newPosition;
				SimplexSolver.Solve(world, remainingTime, minDeltaTime, up, ped.Speed, constraints, ref newPosition, ref newVelocity, out float integratedTime);

				// Apply impulses to hit bodies
				if (affectBodies)
				{
					CalculateAndStoreDeferredImpulses(World,stepInput, characterMass, prevVelocity, ref constraints, ref deferredImpulseWriter);
				}
		...
		private static void CreateConstraintFromHit([ReadOnly] PhysicsWorld world, int rigidBodyIndex, ColliderKey colliderKey,
		...
		private static unsafe void CalculateAndStoreDeferredImpulses(PhysicsWorld World,
			EntityControllerStepInput stepInput, float characterMass, float3 linearVelocity,
			ref NativeList<SurfaceConstraintInfo> constraints, ref NativeStream.Writer deferredImpulseWriter)
		{
			...
			for (int i = 0; i < constraints.Length; i++)
			{
				...
				float deltaVelocity = -projectedVelocity * stepInput.LinearDamping;
